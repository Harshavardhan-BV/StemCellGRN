...
function asyncUpdate(update_matrix::Array{Int,2},
    nInit::Int, nIter::Int, SigStrength, EdgeWt)
    ...
    for j in 1:n_nodes
        update_matrix[n_nodes,j] = update_matrix[n_nodes,j] * SigStrength
    end
    update_matrix2 = 2*update_matrix + Matrix(I, n_nodes, n_nodes)
    update_matrix2 = sparse(update_matrix2')
    
    update_matrix1 = update_matrix
    for j in 2:n_nodes
        update_matrix1[1,j] = update_matrix[1,j] * EdgeWt
    end
    update_matrix3 = 2*update_matrix1 + Matrix(I, n_nodes, n_nodes)
    update_matrix3 = sparse(update_matrix3')
    @showprogress for i in 1:nInit
        ...
        for j in 1:nIter
            ...
            if state[1] == -1
                ...
                s1 = sign.(update_matrix3*state)
            end
            u = rand(1:n_nodes-1, 1)
