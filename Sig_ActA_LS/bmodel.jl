...
function bmodel(topoFile::String; nInit::Int64=10000, nIter::Int64=1000,
    ...
    negativeOdd::Bool = false, SigStrength::Int64, EdgeWt::Int64)
        ...
        if nonMatrix
            ...
            if stateRep == -1
                if randSim
                    ...
                    state_df, frust_df = asyncUpdate(update_matrix, nInit, nIter, SigStrength, EdgeWt)
                end
            else
...
function bmodel_reps(topoFile::String; nInit::Int64=10000, nIter::Int64=1000,
    ...
    write::Bool = true, getData::Bool = false, SigStrength::Int64, EdgeWt::Int64)
    ...
    for type in types
        ...
        for rep in 1:reps
            states_df, Nodes, frust_df = bmodel(topoFile, nInit = nInit, 
                ...
                oddLevel = oddLevel, negativeOdd = negativeOdd, SigStrength = SigStrength, EdgeWt = EdgeWt)
    ...
    if write
        ...
        rootName = join([rootName, "_Sig_Str_", string(SigStrength),"_LS_",string(EdgeWt)])
