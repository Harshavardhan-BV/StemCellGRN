...
function asyncUpdate(update_matrix::Array{Int,2},
    nInit::Int, nIter::Int, Tn_file_name::String)
    ...
    frustVec = []
    timeVec = []
    steady_states = DataFrame(CSV.File(join(["/Volumes/A/MTech Project/Boolean/Boolean.jl-main/Relative_freq_SS/", Tn_file_name, "_finFlagFreq.csv"]))).states
    push!(steady_states, join(["'", lpad(string(0), n_nodes, '0'), "'"]), join(["'", lpad(string(1), n_nodes, '1'), "'"]))
    ...
    @showprogress for i in 1:length(steady_states) * 500
        state_index = rand(1:length(steady_states))
        #println("Init")
        init = steady_states[state_index]
        init = string(init)
        #state = rand(stateVec, n_nodes) #pick random state
        state = zeros(Int, n_nodes)
        for node_index in 1:n_nodes
            if parse(Int, init[node_index + 1]) == 0
                state[node_index] = stateVec[1]
            elseif parse(Int, init[node_index + 1]) == 1
                state[node_index] = stateVec[2]
            else
                println("something wrong")
            end
        end

        perturb_node_index = rand(1:n_nodes)
        if state[perturb_node_index] == -1
            state[perturb_node_index] = 1
        elseif state[perturb_node_index] == 1
            state[perturb_node_index] = -1
        else
            println("something wrong while perturbing node")
        end
